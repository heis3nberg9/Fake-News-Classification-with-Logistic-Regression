...
This project builds a fake news classifier using Python and Natural Language Processing (NLP). The model is trained on a labeled dataset of real and fake news articles, learning to detect patterns in the text that distinguish credible reporting from misinformation. Given a news headline or article, it predicts whether the content is REAL or FAKE. The pipeline uses TF-IDF vectorization to convert raw text into meaningful numeric features, and Logistic Regression as the classification model achieving a test accuracy of 92%.

It is important to note that this model functions as a stylistic classifier rather than a factual truth-checker. Because it was trained specifically on structured news datasets, it learns to associate formal, calm language with "Real News" and sensationalist, emotional language with "Fake News." So when presented with gibberish or random character strings, the model may classify them as "REAL." This occurs because the random input lacks the specific linguistic triggers (like clickbait keywords or extreme punctuation) that the model associates with misinformation. This highlights a core challenge in NLP: the model is highly effective within the context of news-like text but lacks a "rejection" class for non-linguistic or nonsensical input.
